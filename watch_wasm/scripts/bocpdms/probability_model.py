...
class ProbabilityModel:
    ...
    def update_joint_log_probabilities(
        ...
        if self.generalized_bayes_rld == "power_divergence":
            ...
            if len(integral_exceeds) > 0:
                integral_exp[integral_exceeds] = min(
                    1.0, (1.0 / (self.alpha_rld + 1.0))
                ) * (np.finfo(np.float64).max)
            if len(integral_fine) > 0:
                """Just do the standard thing"""
                integral_exp[integral_fine] = (1.0 / (self.alpha_rld + 1.0)) * np.exp(
                    integrals[integral_fine]
                )
            ...
            step_ahead_exceeds = np.where(
                self.one_step_ahead_predictive_log_probs >= max_val
            )[0]
            ...
            if len(step_ahead_exceeds) > 0:
                if self.alpha_rld < 1:
                    ...
                    step_ahead_exp[step_ahead_exceeds] = min(
                        1.0, 1.0 / (self.alpha_rld)
            ...
            if len(step_ahead_fine) > 0:
                """Just do the standard thing"""
                step_ahead_exp[step_ahead_fine] = (1.0 / (self.alpha_rld)) * np.exp(
                    self.one_step_ahead_predictive_log_probs[step_ahead_fine]
                    * self.alpha_rld
                )
            ...
            if (
                ...
                and do_general_bayesian_hyperparameter_optimization
                and disable_hyperparameter_optimization is not True
            ):
                """updates the log of the joint probabilities' derivatives
                with respect to alpha. probability_model level function"""
                self.update_alpha_derivatives(
                    y,
                    t,
                    log_model_posteriors,
        ...
        self.joint_log_probabilities = np.insert(
            growth_log_probabilities, 0, CP_log_prob
        )
        ...
        if (
            self.hyperparameter_optimization is not None
            and self.hyperparameter_optimization is not False
            and disable_hyperparameter_optimization is not True
        ):
            ...
            gradients_log_predictive_val, gradients_log_predictive_sign = (
                self.differentiate_predictive_log_distribution(
                    y, t, run_length_log_distro
                )
            )
            ...
            model_specific_joint_log_probs = joint_log_probabilities_tm1 - np.log(
                model_prior
            )
            ...
            Size is num_params x run_length_num
            """
            results_grad_1 = scipy.special.logsumexp(
                a=np.array(
                    [
            ...
            CP_grad_1_val, CP_grad_1_sign = (
                results_grad_1[0].flatten(),
                results_grad_1[1].flatten(),
            )
            results_grad_2 = scipy.special.logsumexp(
                a=np.array(
                    [
                        self.model_specific_joint_log_probabilities_derivative
                        + np.log(cp_model.hazard_vector(1, t))
            ...
            CP_grad_2_val, CP_grad_2_sign = (
                results_grad_2[0].flatten(),
            ...
            if (
                self.hyperparameter_optimization == "caron"
                or self.hyperparameter_optimization == "online"
            ):

                """STEP 6.1: Get sum of the model specific joint log prob
                derivatives, i.e. sum_{r \in R(t)} d/dtheta P(r_t, y_1:t|m_t)"""
                joint_log_der_sum, joint_log_der_sum_signs = scipy.special.logsumexp(
                ...
                rld_der_signs, rld_der = scipy.special.logsumexp(
                    ...
                    b=np.array(
                        [
                            self.model_specific_joint_log_probabilities_derivative_sign,
                            (-1)
                            * joint_log_der_sum_signs[:, np.newaxis]
                ...
                gradient, gradient_signs = scipy.special.logsumexp(
                    ...
                    b=np.array([gradients_log_predictive_sign[:, 1:], rld_der_signs]),
                    return_sign=True,
                    axis=(0, 2),
                )
                ...
                pred = scipy.special.logsumexp(
                    self.one_step_ahead_predictive_log_probs + run_length_log_distro
                )
                ...
                caron_gradient = (
                    np.minimum((grad), pow(10, 5) * np.ones(num_params)) * sig
                )
                #                print(pred)
                #                print(gradient)
                #                print(gradient)
            ...
            self.model_specific_joint_log_probabilities_derivative_sign = np.insert(
                res_sign, 0, CP_grad_sign, axis=1
            )

    def update_alpha_derivatives(
        self,
        y,
        t,
        log_model_posteriors,
        ...
        CP_d_1 = (
            one_step_ahead_log_loss_derivatives[0]
            + np.log(model_prior)
            + log_CP_evidence
        )
        ...
        CP_derivative, CP_derivative_sign = scipy.special.logsumexp(
            ...
            b=np.array(
                [one_step_ahead_log_loss_derivatives_sign[0], log_CP_evidence_der_sign]
            ),
            return_sign=True,
        )
    ...
    def trim_run_length_log_distrbution(self, t, threshold, trim_type):
        ...
        if not ((threshold is None) or (threshold == 0) or (threshold == -1)):
            ...
            run_length_log_distr = (
                self.joint_log_probabilities - self.model_log_evidence
            )
            ...
            if trim_type == "threshold":
                ...
                kept_run_lengths = np.full(
                    np.size(run_length_log_distr), False, dtype=bool
                )
                ...
                for index, rl_particle in zip(
                    range(0, np.size(run_length_log_distr)), run_length_log_distr
                ):
                    if rl_particle < alpha:
                        ...
                        if u <= rl_particle:
                            ...
                            # DEBUG: log sum exp
                            u = scipy.special.logsumexp(np.array([u, alpha, -rl_particle]))
            ...
            if trim_type == "SRC" and np.sum(kept_run_lengths) > 1:
                ...
                self.joint_log_probabilities = (
                    run_length_log_distr + self.model_log_evidence
                )
    ...
    def DPD_joint_log_prob_updater(
        self,
        alpha_direction,
        y,
        t,
        ...
        r0_log_prob = self.evaluate_log_prior_predictive(
            y, t, store_posterior_predictive_quantities=False
        )
        # IMPLEMENT: Needs to be adapted s.t. I can get evaluations for
        #           the different sets of parameters (i.e. L_rt_p_eps etc)
        one_step_ahead_predictive_log_probs = self.evaluate_predictive_log_distribution(
            y,
            t,
        ...
        if self.generalized_bayes_rld == "power_divergence":
            ...
            if len(integral_exceeds) > 0:
                integral_exp[integral_exceeds] = min(
                    1.0, (1.0 / (self.alpha_rld + 1))
                ) * (np.finfo(np.float64).max)
            if len(integral_fine) > 0:
                """Just do the standard thing"""
                integral_exp[integral_fine] = min(
                    1.0, (1.0 / (self.alpha_rld + 1))
            ...
            if self.r0_log_prob >= max_val:
                ...
                r0_log_prob_exp = (
                    1.0 / self.alpha_rld * np.exp(r0_log_prob * self.alpha_rld)
                )

            """STEP 2.2: Correct other probs"""
            step_ahead_exceeds = np.where(
                one_step_ahead_predictive_log_probs >= max_val
            )[0]
            ...
            if len(step_ahead_fine) > 0:
                """Just do the standard thing"""
                step_ahead_exp[step_ahead_fine] = (1.0 / (self.alpha_rld + 1)) * np.exp(
                    one_step_ahead_predictive_log_probs[step_ahead_fine]
                    * self.alpha_rld
                )
