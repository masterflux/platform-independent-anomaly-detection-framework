...
from bocpdms.BVARNIGDPD import BVARNIGDPD
...
class Detector:
    ...
    def __init__(
        ...
        *data*. E.g., if you have a SxS spatial lattice with T time points,
        ...
        self.model_and_run_length_log_distr = -np.inf * np.ones(
            shape=(self.Q, self.T + 1)
        )
        ...
        for model in self.model_universe:
            if model.has_lags:
                self.smallest_lag_length = min(
                    self.smallest_lag_length, model.lag_length
                )
            else:
        ...
        if (alpha_param is not None) and alpha_param_learning == "together":
            ...
            self.gradient_alpha_param = 0
            print(
                "WARNING! You are using DPD for parameter inference "
        ...
        if (alpha_param_learning == "individual") or alpha_param_learning == "together":
        ...
        if loss_der_rld_learning is None:
            ...
            or loss_der_rld_learning == "squared_loss_derivative"
        ):
            loss_der_rld_learning = Detector.squared_loss_derivative
        elif (
            loss_der_rld_learning == "absolute_loss"
            or loss_der_rld_learning == "absolute_loss_derivative"
        ):
    ...
    def next_run(self, y, t):
        ...
        if (
            (not self.not_all_initialized)
            and self.generalized_bayes_rld == "power_divergence"
            and self.alpha_rld_learning
            and self.alpha_rld_opt_t < t
        ):
    ...
    def rescale_DPD_run_length_log_distribution(self, t):
        ...
        if self.jlp_scale is None:
            log_scale_new = (
                np.max([model.joint_log_probabilities for model in self.model_universe])
                - 1
            )
            ...
            #                        for model in self.model_universe]) - 1)
            min_ = (
                np.min([model.joint_log_probabilities for model in self.model_universe])
                - 1
            )
    ...
    def update_alpha_param(self, y, t, update=True):
        ...
        if self.alpha_param_learning == "together":
            ...
            model_posteriors_p_eps = np.exp(
                np.array(list_model_log_evidences_p_eps) - total_evidence_p_eps
            )
            model_posteriors_m_eps = np.exp(
                np.array(list_model_log_evidences_m_eps) - total_evidence_m_eps
            )
            """STEP 1A.2.2: Get the posterior mean"""
            post_mean_p_eps = (
                np.array(list_post_mean_p_eps) * model_posteriors_p_eps[:, np.newaxis]
            )
            post_mean_m_eps = (
                np.array(list_post_mean_m_eps) * model_posteriors_m_eps[:, np.newaxis]
            )
            ...
            if update:
                ...
                self.alpha_param = min(
                    max(
                        ...
                        * np.sign(
                            self.gradient_alpha_param
                        ),  # step_size * gradient_alpha_param
                    ),
                    10.0,
                )
                self.gradient_alpha_param = 0
    ...
    def update_run_length_log_distribution(self, t):
        ...
        for index in indices_initialized_models:
            ...
            model_rl_log_distributions[
                index, model_indices_indicators_relative_to_all_run_lengths
        ...
        run_length_log_distr = scipy.special.logsumexp(model_rl_log_distributions, axis=0)
    ...
    def update_all_joint_log_probabilities(self, y, t):
        ...
        if (
            ...
            at_least_one_model_initialized = np.any(
                [
                    (model.has_lags and ((t) - model.lag_length) > 1)
                    or (not model.has_lags)
                    for model in self.model_universe
                ]
            )
            ...
            if at_least_one_model_initialized:
                ...
                for m, model in zip(range(0, self.Q), self.model_universe):
                    ...
                    if warmed_up:
                        ...
                        if (
                            model.retained_run_lengths[-1]
                            == model.retained_run_lengths[-2]
                        ):
                            model_indices_indicators_relative_to_all_run_lengths[-1] = (
                                True
                            )
                ...
                model_sums_derivatives, model_sums_derivatives_sign = scipy.special.logsumexp(
                ...
                *joint_probabilities*, (ii) the predictive distributions (i.e.,
                ...
                if model.has_lags or isinstance(model, BVARNIG):
                    ...
                    if model.exo_bool:
                        ...
                        X_exo_2 = self.exo_data[
                            model.lag_length + 1, model.exo_selection, :
                        ]
                    else:
                        X_exo = X_exo_2 = None
                ...
                if warmed_up:
                    ...
                    log_model_posteriors = log_model_posteriors[
                        np.where(log_model_posteriors > -np.inf)
                    ]
                    ...
                    if (
                        ...
                        log_model_posteriors_der_m = log_model_posteriors_der[
                            m, model_indices_indicators_relative_to_all_run_lengths
                        ]
                        log_model_posteriors_der_sign_m = log_model_posteriors_der_sign[
                            m, model_indices_indicators_relative_to_all_run_lengths
                        ]
                ...
                if model.has_lags and ((t) - model.lag_length) > 1:
                    ...
                    if (t - model.lag_length) > 2 and self.alpha_param_opt_t <= t:
                        ...
                        model.alpha_param_gradient_computation(
                            y=y,
                            t=t,
                            cp_model=self.cp_model,
                            model_prior=self.model_prior[m] / prior_rescaling_factor,
                            log_model_posteriors=log_model_posteriors,
                    ...
                    model.update_joint_log_probabilities(
                        ...
                        log_model_posteriors_der_sign=log_model_posteriors_der_sign_m,
                ...
                if model.has_lags and ((t) - model.lag_length) > 1:
                    ...
                    if isinstance(model, BVARNIGDPD):
                        ...
                        model.update_predictive_distributions(
                            y, y_tm1, x_exo_t, x_exo_tp1, t, self.cp_model.hazard(0)
                        )
                    else:
                        """If not DPD model, we don't need CP probability"""
                        model.update_predictive_distributions(
                            y, y_tm1, x_exo_t, x_exo_tp1, t
                        )
                elif not model.has_lags:
                    if isinstance(model, BVARNIG):
    ...
    def update_alpha_rld(self, y, t, update=True):
        ...
        if True:  # self.loss_type == "posterior_expectation":
            ...
            for m, model in zip(range(0, self.Q), self.model_universe):
                ...
                post_mean_der = post_mean_der + np.sum(
                    np.reshape(
                        model.get_posterior_expectation(t),
        ...
        if False:  # self.loss_type = "posterior_expectation"
            ...
            for m, model in zip(range(0, self.Q), self.model_universe):
                ...
                if np.sum(
                    ...
                    one_steps = np.insert(
                        model.one_step_ahead_predictive_log_probs, 0, model.r0_log_prob
                    )
                else:
                ...
                post_prob_log = scipy.special.logsumexp(a=np.array([post_prob_log, sum_]))
                """rlm log derivative"""
        ...
        self.gradient_alpha_rld = self.gradient_alpha_rld + self.loss_der_rld_learning(
            resid.flatten(), post_mean_der.flatten(), self.C
        )
        ...
        if update:
            ...
            self.alpha_rld = min(
                max(self.alpha_rld - increment * grad_sign, min_alpha), max_alpha
            )
    ...
    def save_negative_log_likelihood(self, t):
        ...
        all_one_step_ahead_lklhoods_weighted = [
            (
                ...
                + self.model_and_run_length_log_distr[
                    m, self.model_and_run_length_log_distr[m, :] > -np.inf
                ]
            )
        ...
        summed_up = -scipy.special.logsumexp(
            np.array(
                [
                    item
                    for entry in all_one_step_ahead_lklhoods_weighted
                    for item in entry
                ]
            )
        )
    ...
    def save_negative_log_likelihood_fixed_pars(self, t):
        ...
        summed_up = -special.logsumexp(
            [item for entry in all_one_step_ahead_lklhoods_weighted for item in entry]
        )
    ...
    def update_log_evidence(self):
        ...
        self.log_evidence = scipy.special.logsumexp(
            np.array([model.model_log_evidence for model in self.model_universe])
        )
    ...
    def prediction_y(self, y, t):
        ...
        post_mean, post_var = (
            np.zeros(shape=(self.S1, self.S2)),
        ...
        for m, model in zip(range(0, self.Q), self.model_universe):
            ...
            """weighing expectation with model & run-length probabilities"""
            post_mean = post_mean + np.sum(
                np.reshape(
                    model.get_posterior_expectation(t),
                    newshape=(num_rl, self.S1, self.S2),
                )
                * np.exp(model.joint_log_probabilities - self.log_evidence)[
            ...
            post_var = post_var + np.sum(
                np.reshape(
                    model.get_posterior_variance(t),
                    newshape=(num_rl, self.S1 * self.S2, self.S1 * self.S2),
                )
                * np.exp(model.joint_log_probabilities - self.log_evidence)[
                    :, np.newaxis, np.newaxis
                ],
        ...
        for model in self.model_universe:
            ...
            if model.has_lags:
                if model.lag_length < t:
                    model.trim_run_length_log_distrbution(
                        t, self.threshold, self.trim_type
                    )
            else:
    ...
    def update_priors(self, t):
        ...
        for m, model in zip(range(0, self.Q), self.model_universe):
            if model.auto_prior_update == True:
                ...
                model_specific_rld = self.model_and_run_length_log_distr[
                    m, :
    ...
    def MAP_estimate(self, t):
        ...
        *log_MAP_proposed* quantity implied by that model will beat the current 
        ...
        self.log_MAP_storage = np.append(
            self.log_MAP_storage, log_MAP_current
        )  # [t-1] = log_MAP_current #problem log map storage
        # indices are not aligned with the run-lengths?!

        if (
            CP_initialization
        ):  # or (t == r_star + self.model_universe[m_star].lag_length + 1):
