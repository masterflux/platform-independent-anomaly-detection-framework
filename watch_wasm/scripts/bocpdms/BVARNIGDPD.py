...
class BVARNIGDPD(BVARNIG):
    ...
    def initialization(
        ...
        Y1 = X_endo[-1, :].flatten()
        Y2 = Y_2.flatten()
        X1_endo = X_endo[: self.lag_length, :].reshape(
            self.lag_length, self.S1, self.S2
        )
        ...
        """
        self.XX_t = np.zeros(
            shape=(self.VB_window_size, self.num_regressors, self.num_regressors)
        )
        ...
        self.compute_X_XX_XY_YY(
            Y1, X1_endo, X1_exo, padding_columns_computeXX, compute_XY=True
        )
        ...
        self.SVRG_anchor_gradient_indices = [
            np.array([0])
        ]  # NOT extended in the first iteration
        self.SVRG_anchor_sum = [
            np.zeros(
                int(self.num_regressors * (self.num_regressors + 1) * 0.5)
                + self.num_regressors
                + 2
        ...
        self.full_VB_optimizer(
            r=0, num_obs=1, t=self.lag_length + 1, cp_prob=1.0, alpha=self.alpha_param
        )
        """Take care of r=t-1 & r>t-1 manually (usually done in the loops 
        later on)"""
        self.anchor_params[-1] = self.anchor_params[0].copy()  # r=t-1 & r>t-1
        self.SVRG_anchor_gradient_indices[0] = self.SVRG_anchor_gradient_indices[
            -1
        ].copy()
        ...
        Q_0, R_0 = np.linalg.qr(
            np.identity(self.S1 * self.S2) + np.matmul(XL, np.transpose(XL))
        )
        ...
        self.model_log_evidence = np.log(model_prior) + BVARNIG.mvt_log_density(
            resid, C_0_inv, C_0_log_det, 2 * self.a_rt[0], self.non_spd_alerts
        )
    ...
    def full_VB_optimizer(self, r, num_obs, t, cp_prob, alpha=None):
        ...
        eps = pow(10, -5)
        self.bounds = (
            [(1 + eps, self.a + int(num_obs * 0.75))]
            + [(1 + eps, None)]
            + [(None, None)] * (p + int(p * (p + 1) * 0.5))
        )
        ...
        if not self.full_opt_thinning_schedule is None:
            need_full_opt = (self.retained_run_lengths[r] == 0) or (
                (self.retained_run_lengths[r] in self.full_opt_thinning_schedule)
        ...
        #                                    SCSG = False)
            ...
            self.SVRG_VB_optimizer(
                ...
                alpha,
                anchor_approx_goodness=min(
                    self.VB_window_size - 1, self.anchor_batch_size_SCSG
                ),
                SCSG=True,
            )
    ...
    def SVRG_VB_optimizer(
        ...
        last_full_opt = False
        if not self.full_opt_thinning_schedule is None:
            last_full_opt = (run_length - 1) in self.full_opt_thinning_schedule
        elif not self.full_opt_thinning is None:
            last_full_opt = (run_length - 1) % self.full_opt_thinning == 0
        # if self.retained_run_lengths[r] == self.first_full_opt:
        #    last_full_opt = True

        if last_full_opt or (  # ((run_length - 1) % self.full_opt_thinning == 0)
            ...
            if anchor_approx_goodness is None:
                ...
                num_ind = min(
                    min(anchor_approx_goodness, run_length), self.VB_window_size - 1
                )
                rl_ = min(run_length, self.VB_window_size - 1)
                indices = np.random.choice(rl_, size=num_ind, replace=False)
            ...
            args = (
                self.a,
                self.b,
                self.prior_mean_beta,
                self.prior_var_beta_inv,
                alpha,
                len(self.SVRG_anchor_gradient_indices[r_]),
                p,
                self.K,
                self.E3,
                self.Rinv,
            ...
            grads = BVARNIGDPD.get_grad_neg_ELBO_full(params, *args)
        ...
        num_SGs = min(self.SGD_approx_goodness, min(run_length, self.VB_window_size))
        num_steps = min(
            run_length, min(self.VB_window_size, 1)
        )  # DEBUG: Let this potentially be another input!
        ...
        for step_count in range(0, num_steps_this_cycle):
            ...
            indices = np.random.choice(
                max_ind, size=min(num_SGs, max_ind), replace=False
            )  # random draw of size num_SGs + most recent one
            ...
            if step_count == 0:
                ...
                new_obs_grad = BVARNIGDPD.get_grad_neg_ELBO_full(
                    self.anchor_params[r_], *args_new_obs
                ).flatten()

                """STEP 2.2.2.2: Update mu (SVRG_anchor_sum)"""
                self.SVRG_anchor_sum[r_] = (grad_size * w) * self.SVRG_anchor_sum[
                    r_
                ] + w * new_obs_grad
                ...
                self.SVRG_anchor_gradients[r_] = np.insert(
                    self.SVRG_anchor_gradients[r_], 0, new_obs_grad, axis=0
                )
                ...
                self.SVRG_anchor_gradient_indices[r_] = np.insert(
                    self.SVRG_anchor_gradient_indices[r_] + 1, 0, 0
                )
            ...
            new_batch_grad = (1 / np.size(indices)) * BVARNIGDPD.get_grad_neg_ELBO_full(
                params, *args_new_grad
            )
            ...
            increment = step_size * (
                new_batch_grad - old_batch_grad + self.SVRG_anchor_sum[r_]
            )
            ...
            if self.a_rt[r] - increment[0] > self.a + int(
                0.75 * self.retained_run_lengths[r]
            ):
                """get ratio of how much a-gradient result would be more efficient
                than KL + modify gradients by downweighting accordinly"""
                ratio = (self.a_rt[r] + increment[0]) / (
                    self.a + int(0.5 * self.retained_run_lengths[r])
                )
            ...
            if np.isnan(increment).any():
                print(
                    "We have a nan increment at position", np.where(increment == True)
                )
            ...
            self.L_rt[r, :][lower_ind] = (
                self.L_rt[r, :][lower_ind] - increment[(p + 2) :]
            )
            ...
            if step_count == 0 and SCSG == True:
                ...
                self.SVRG_anchor_gradients = [
                    self.SVRG_anchor_gradients[i][
                        np.where(
                            self.SVRG_anchor_gradient_indices[i] < self.VB_window_size
                        )
                    ]
                    for i in range(0, l)
                ]
                ...
                #                             self.SVRG_anchor_gradients ]
                self.SVRG_anchor_gradient_indices = [
                    e[np.where(e < self.VB_window_size)]
                    for e in self.SVRG_anchor_gradient_indices
                ]
    ...
    def LBFGSB_optimizer(
        ...
        old_params[0] = max(
            1.00001, self.a_rt[r]
        )  # self.a + int(self.retained_run_lengths[r]*0.5*0.5)) #self.a + int(self.retained_run_lengths[r]*0.5))#self.a_rt[r])
        ...
        if DEBUG_gradients_optimizer:
            ...
            # L
            for i in range(
                0, int(self.num_regressors * (self.num_regressors + 1) * 0.5)
            ):
            ...
            grad2_a = BVARNIGDPD.get_grad_a(old_params, *args)
            grad2_b = BVARNIGDPD.get_grad_b(old_params, *args)
            grad2_beta = BVARNIGDPD.get_grad_beta(old_params, *args)
            grad2_L = BVARNIGDPD.get_grad_L(old_params, *args)
            elbo = BVARNIGDPD.ELBO_fun(old_params, *args)
            elbo_obj = self.get_ELBO(
                perturbation=np.zeros(int(p * (p + 1) * 0.5) + p + 2),
                r=r,
                num_obs=num_obs,
            )
        ...
        if self.retained_run_lengths[r] >= self.first_full_opt:
            ...
            if successful_optimization:
                ...
                new_params[0] = max(
                    1.00001, self.a_rt[r]
                )  # self.a + int(self.retained_run_lengths[r]*0.5))#self.a_rt[r])
        ...
        #                        print(opt_object.message)
            ...
            self.retained_run_lengths[r] < self.first_full_opt
            and self.retained_run_lengths[r] > 0
        ):
            # SGD step
            ...
            self.SGD_VB_optimizer(
                r=r, num_obs=1, t=t, cp_prob=0.5, alpha=alpha, precompute=False
            )
            # get new params
            new_params = np.zeros(int(p * (p + 1) * 0.5) + p + 2)
            new_params[0] = max(
        ...
        if self.retained_run_lengths[r] == 0:
            ...
            self.SVRG_anchor_gradients.insert(
                0, np.zeros(int(p * (p + 1) * 0.5) + p + 2)
            )
    ...
    def SGD_VB_optimizer(self, r, num_obs, t, cp_prob, alpha=None, precompute=True):
        ...
        n = min(
            r, self.SGD_approx_goodness
        )  # min(self.VB_window_size, r+1) #n = number of obs we use to obtain the gradient
        ...
        C = 1 / (1 - (1 / (1 + cp_prob)))  # = 1/(1-p)
        prior_weight = (1.0 / C) * pow(
            1 / (1 + cp_prob), self.retained_run_lengths[r] + 1
        )
        ...
        lamb = max(
            0.01, 1.0 - self.alpha_param
        )  # 0.25 #learning rate decreases at rate (lamb * t)^-1
        step_size = g0 / (1.0 + g0 * lamb * self.retained_run_lengths[r])
        premult = 1.0  # 1.0/pow(self.alpha_param, 1/3)
        step_size_ab = (
            premult * 3.0 / (1.0 + 3.0 * 0.99 * self.retained_run_lengths[r])
        )  # separate learning rate for a and b makes huge difference
        ...
        if self.a_rt[r] + a_increment > self.a + int(
            0.75 * self.retained_run_lengths[r]
        ):
            """get ratio of how much a-gradient result would be more efficient
            than KL + modify gradients by downweighting accordinly"""
            ratio = (self.a_rt[r] + a_increment) / (
                self.a + int(0.5 * self.retained_run_lengths[r])
            )
        ...
        self.L_rt[r, :, :][lower_ind] = (
            self.L_rt[r, :, :][lower_ind] + gradient_L * step_size
        )

    def precomputations_VB_optimizer(
        self,
        r,
        num_obs,
        alpha=None,
        ...
        self.gamma_ratio_2 = np.exp(
            scipy.special.gammaln(an + 0.5 * alpha * d) - scipy.special.gammaln(an)
        )
        self.digamma_diff_1 = scipy.special.digamma(an + 1) - scipy.special.digamma(an)
        self.digamma_diff_2 = scipy.special.digamma(
            an + 0.5 * alpha * d
        ...
        for i in sample_indices:
            ...
            try:
                if p > 1:
                    Li = np.linalg.cholesky(
                        np.matmul(Ln, np.transpose(Ln)) + alpha * self.XX_t[i, :, :]
                    )
                else:
        ...
        for i in sample_indices:
            ...
            self.K[ind] = bn + 0.5 * (alpha * self.YY_t[i] + E4 + E5to7i)
            ind = ind + 1

    """evaluation needs to be formulated differently since we track 
    different objects now"""

    def evaluate_predictive_log_distribution(
        self, y, t, store_posterior_predictive_quantities=True, alpha_direction=None
    ):
        ...
        if store_posterior_predictive_quantities:
            self.C_t_inv = np.zeros(
                (run_length_num + 1, self.S1 * self.S2, self.S1 * self.S2)
            )
        ...
        for r in range(0, run_length_num):
            ...
            if self.S1 * self.S2 > 1 or self.num_regressors > 1:
                ...
                C_t_inv = 1.0 / (1 + XL_sq)
                predictive_variance_log_det = (np.log(b_rt[r]) - np.log(a_rt[r])) * (
                    1 + XL_sq
                )
            ...
            if store_posterior_predictive_quantities:
                ...
                self.predictive_variance_log_det[r + 1] = predictive_variance_log_det
            ...
            log_densities[r] = BVARNIG.mvt_log_density(
                resid,
    ...
    def get_log_integrals_power_divergence(self, DPD_call=False):
        ...
        nu_1 = 2 * np.insert(
            self.a_rt, 0, self.a
        )  # (self.a + (run_length_with_0+1.0)*0.5) self.a_rt * 2
        # if DPD_call:
        #    nu_1 = 2 * self.a_rt
        nu_2 = nu_1 * self.alpha_rld + p * self.alpha_rld + nu_1

        C1 = (1.0 + self.alpha_rld) * (
            special.gammaln(0.5 * (nu_1 + p)) - special.gammaln(0.5 * nu_1)
        )
        ...
        return (
            C1
            - C2
            - nu_1 * 0.5 * p * self.alpha_rld
            - np.pi * 0.5 * p * self.alpha_rld  # dets)
            - self.alpha_rld * pred_var
        )  # self.predictive_variance_log_det )

    def evaluate_log_prior_predictive(
        self, y, t, store_posterior_predictive_quantities=True
    ):
        ...
        if self.S1 * self.S2 > 1 or self.num_regressors > 1:
            ...
            C_t_inv_r0 = np.matmul(R_inv, np.transpose(Q))
            ...
            predictive_variance_r0_log_det = (np.log(self.b) - np.log(self.a)) * (
                XL_sq + 1
            )
    ...
    def update_predictive_distributions(
        self,
        y_t,
        y_tm1,
        x_exo_t,
        ...
        self.regressor_cross_product_updates(
            y_t,
            y_tm1,
            x_exo_t,
            t,
        ...
        if self.dynamic_VB_window_size and rl >= wnd:
            ...
            XX_new = np.zeros(
                (self.VB_window_size, self.num_regressors, self.num_regressors)
            )
        ...
        self.successful_terminations = np.insert(
            self.successful_terminations.copy(), 0, False
        )
        self.a_rt = np.insert(self.a_rt.copy(), 0, self.a_rt[0].copy())
        self.b_rt = np.insert(self.b_rt.copy(), 0, self.b_rt[0].copy())
        self.beta_rt = np.insert(
            self.beta_rt.copy(), 0, self.beta_rt[0, :].copy(), axis=0
        )
        ...
        for r in range(0, unique_run_length_num):
            ...
            self.full_VB_optimizer(
                r=r, num_obs=num_obs, t=t, cp_prob=cp_prob, alpha=self.alpha_param
            )
            # else:
            ...
            if have_double and r == unique_run_length_num - 1:
                ...
                self.SVRG_anchor_gradients[-1] = self.SVRG_anchor_gradients[-2].copy()
        ...
        if self.alpha_param_learning:
            ...
            self.SVRG_anchor_gradient_indices = (
                retained_pre_SVRG_anchor_gradient_indices.copy()
            )
            ...
            for r in range(0, unique_run_length_num):
                ...
                self.full_VB_optimizer(
                    r=r, num_obs=num_obs, t=t, cp_prob=cp_prob, alpha=alpha_p
                )
                #                else:
            ...
            self.a_rt_p_eps, self.b_rt_p_eps = (self.a_rt.copy(), self.b_rt.copy())
            self.anchor_params = retained_pre_anchor_params.copy()
            self.SVRG_anchor_gradient_indices = (
                retained_pre_SVRG_anchor_gradient_indices.copy()
            )
            ...
            for r in range(0, unique_run_length_num):
                ...
                self.full_VB_optimizer(
                    r=r, num_obs=num_obs, t=t, cp_prob=cp_prob, alpha=alpha_m
                )
                #                else:
            ...
            self.a_rt_m_eps, self.b_rt_m_eps = (self.a_rt.copy(), self.b_rt.copy())
    ...
    def alpha_param_gradient_computation(
        self, y, t, cp_model, model_prior, log_model_posteriors, log_CP_evidence, eps
    ):
        ...
        if self.alpha_param_learning:
            ...
            self.alpha_param_m_eps = max(pow(10, -10), self.alpha_param - eps)
            ...
            self.model_log_evidence_p_eps = scipy.misc.logsumexp(joint_log_probs)
            ...
            self.post_mean_p_eps = np.sum(
                np.reshape(
                    np.matmul(
                        self.X_tp1,
                        np.insert(self.beta_rt_p_eps, 0, self.prior_mean_beta, axis=0)[
                            :, :, np.newaxis
                        ],
                    ),
                    newshape=(self.retained_run_lengths.shape[0] + 1, self.S1, self.S2),
                )
                * (
            ...
            """STEP 3.2: as before"""
            self.model_log_evidence_m_eps = scipy.misc.logsumexp(joint_log_probs)
            ...
            self.post_mean_m_eps = np.sum(
                np.reshape(
                    np.matmul(
                        self.X_tp1,
                        np.insert(self.beta_rt_m_eps, 0, self.prior_mean_beta, axis=0)[
                            :, :, np.newaxis
                        ],
                    ),
                    newshape=(self.retained_run_lengths.shape[0] + 1, self.S1, self.S2),
    ...
    def get_posterior_variance(self, t, r_list=None):
        ...
        post_var = np.zeros(
            (np.size(self.retained_run_lengths), self.S1 * self.S2, self.S1 * self.S2)
        )
    ...
    def trimmer(self, kept_run_lengths, BAR_submodel=False):
        ...
        if (
            ...
            and self.log_alpha_derivatives_joint_probabilities is not None
        ):
            self.log_alpha_derivatives_joint_probabilities = (
                self.log_alpha_derivatives_joint_probabilities[kept_run_lengths]
            )
            self.log_alpha_derivatives_joint_probabilities_sign = (
                self.log_alpha_derivatives_joint_probabilities_sign[kept_run_lengths]
            )
        ...
        if True:
            ...
            self.SVRG_anchor_gradients = [
                self.SVRG_anchor_gradients[ind] for ind in keepers
            ]
        ...
        if self.dynamic_VB_window_size and wnd > rl and self.max_rl >= 100:
            # new_len = 100 * (rl + 1)
    ...
    def ELBO_fun(params, *args):
        ...
        E1 = (
            -np.log(abs(np.prod(np.diag(L0))))
            - np.log(abs(np.prod(np.diag(Ln))))
            + 0.5
            * (p - np.trace(np.matmul(Sig0inv, np.matmul(Linv, np.transpose(Linv)))))
        )
        ...
        E8 = -(
            ...
            * an
            * (
                np.inner(beta0 - betan, np.matmul(Sig0inv, beta0 - betan))
                + 2 * (b0 - bn)
            )
        )
        ...
        if num_obs > 0:
            log_sum = True
            if not log_sum:
                sum_part = pow(bn, -0.5 * d * alpha) * np.sum(
                ...
                sum_, sign_ = scipy.misc.logsumexp(
                    a=np.log(np.abs(E3))
                    + (an + 0.5 * d * alpha) * (np.log(bn) - np.log(np.abs(K))),
                    b=sign,
                    return_sign=True,
                )
    ...
    def precompute_VB(params, *args):
        ...
        digamma_diff_1 = scipy.special.digamma(an + 1) - scipy.special.digamma(an)
        digamma_diff_2 = scipy.special.digamma(
            an + 0.5 * alpha * d
        ) - scipy.special.digamma(an)
        ...
        for i in range(0, num_obs):  # specified_indices:
            ...
            try:
                if p > 1:
                    Li = np.linalg.cholesky(
                        np.matmul(Ln, np.transpose(Ln)) + alpha * XX_t[dind, :, :]
                    )
                else:
        ...
        for i in range(0, num_obs):  # specified_indices:
            ...
            K[i] = bn + 0.5 * (alpha * YY_t[dind] + E4 + E5to7i)
    ...
    def get_grad_a(params, *args):
        ...
        E10der = (
            np.log(bn)
            - scipy.special.digamma(an)
            - (an - a0) * scipy.special.polygamma(1, an)
        )
        E11der = -(
            (num_obs * gamma_ratio_2 * digamma_diff_2)
            * pow(bn * 2.0 * np.pi, -0.5 * d * alpha)
            * pow(1.0 + alpha, -0.5 * d - 1)
        )
            ...
            if return_sum:
                if not log_sum:
                    sum_1 = (
                        E2der_m_ba
                        * downweight1
                        * np.sum(E3 * np.power(bn / K, an + 0.5 * d * alpha))
                    )

                    Klarger0 = np.where(K > 0)
                    Ksmaller0 = np.where(K <= 0)
                    exact_der_part = np.sum(
                        E3[Klarger0]
                        * np.power(
                            bn / K[Klarger0],  # np.maximum(K, pow(10,-5)),
                            an + 0.5 * d * alpha,
                        )
                        # DEBUG: Force K > 0
                    ...
                    sum_1_log, sign = scipy.misc.logsumexp(  # E2der_m_ba * (
                        a=(
                            np.log(np.abs(E3))
                    ...
                    sum_1 = (
                        sign
                        * np.exp(sum_1_log - np.log(bn) * 0.5 * d * alpha)
                        * E2der_m_ba
                    )
                    ...
                    if np.size(Ksmaller0):
                        ...
                        diff_log, diff_log_sign = scipy.misc.logsumexp(
                            ...
                            b=np.array(
                                [np.sign(K[Ksmaller0]), (-1) * np.sign(K[Ksmaller0])]
                            ),
                            return_sign=True,
                            axis=0,
                        )
                    ...
                    sum_part, sum_sgn = scipy.misc.logsumexp(
                        a=np.array([exact_der_part_log, numerical_der_part_log]),
                    ...
                    sum_2 = (
                        ...
                        * (-1.0)
                        * sum_sgn
                        * np.exp(sum_part - (0.5 * d * alpha) * np.log(bn))
                    )
            if not return_sum:
                ...
                exact_der_part = (
                    E3[Klarger0]
                    * np.power(bn / K[Klarger0], an + 0.5 * d * alpha)
                    * np.log(K[Klarger0])
                )
                ...
                individual_grads_a = (sum_part_1 + sum_part_2) + prior_weight * (
                    E8der + E9der + E10der + E11der
                )
    ...
    def get_grad_b(params, *args):
        ...
        E10der = (an - a0) / bn
        E10der = E9der + E10der
        E11der = ((num_obs * 0.5 * d * alpha) * gamma_ratio_2) * (
            pow(2 * np.pi, -0.5 * d * alpha)
            * pow(1 + alpha, -0.5 * d - 1)
            * downweight2
        )
        ...
        if num_obs > 0:
            ...
            if return_sum:
                ...
                if not log_sum:
                    ...
                    sum_2 = (
                        ...
                        * (-an - 0.5 * d * alpha)
                        * downweight2
                        * np.sum(E3 * np.power(bn / K, an + 0.5 * d * alpha + 1))
                    )
                else:
                    ...
                    sum_2_log, sign_ = scipy.misc.logsumexp(
                        a=(
                            np.log(np.abs(E3))
                            + (an + 0.5 * alpha + 1) * (np.log(bn) - np.log(np.abs(K)))
                        ),
                        b=sign,
                        axis=0,
                        return_sign=True,
                    ...
                    sum_2 = (
                        sign_
                        * np.exp(sum_2_log - np.log(bn) * (0.5 * d * alpha + 1))
                        * (-an - 0.5 * d * alpha)
                        * E2_m_ba
                    )
            ...
            if not return_sum:
                ...
                individual_grads_b = (
                    prior_weight * (E8der + E10der + E11der) + sum_1_part + sum_2_part
                )
    ...
    def get_grad_beta(params, *args):
        ...
        E4der = 2 * np.matmul(np.matmul(np.transpose(betan), Ln), np.transpose(Ln))
        E8der = (
            (1.0 / bn)
            * an
            * np.matmul(
        ...
        if num_obs > 0:
            ...
            for i in range(0, num_obs):
                ...
                E5der[i, :] = -np.matmul(E5der[i, :], np.transpose(Ln))
                # INEFFICIENT: R^-1 * Sigma^-1 mult. twice! once for E5der before
                E7der[i, :] = np.matmul(np.transpose(XY_t[dind, :]), Rinv[i, :, :])
            ...
            if return_sum:
                ...
                if not log_sum:
                    sum_1 = (
                        ...
                        * (
                            np.sum(
                                E3[:, np.newaxis]
                                * (E4der + E5der + E7der)
                                * np.power(bn / K, an + 0.5 * d * alpha + 1)[
                                    :, np.newaxis
                                ],
                                axis=0,
                            )
                    ...
                    sum_1 = (
                        sign_
                        * np.exp(sum_1_log - np.log(bn) * (0.5 * d * alpha + 1))
                        * (-an - 0.5 * d * alpha)
                        * 0.5
                        * E2_m_ba
                    )
            if not return_sum:
                ...
                sum_1_part = (
                    ...
                    * (
                        E3[:, np.newaxis]
                        * (E4der + E5der + E7der)
                        * np.power(bn / K, an + 0.5 * d * alpha + 1)[:, np.newaxis]
                    )
                )
    ...
    def get_grad_L(params, *args):
        ...
        E1der = np.matmul(
            (
                np.matmul(np.matmul(np.transpose(L_inv), L_inv), Sig0inv)
                - np.identity(p)
            ),
            np.transpose(L_inv),
        )[lower_ind]
        ...
        if num_obs > 0:
            ...
            for i in range(0, num_obs):
                ...
                A6 = np.matmul(np.identity(p) - A2, betan)  # vector!
                ...
                E5ider = (
                    2
                    * np.matmul(
                        (np.matmul(A2 - np.identity(p), np.transpose(A3)) - A3), Ln
                    )[lower_ind]
                )
                ...
                E7ider = (
                    -2
                    * alpha
                    * (np.outer(A6, A5) + np.matmul(np.outer(A4, A6), Ln))[lower_ind]
                )
            ...
            if return_sum:
                ...
                full_sum = (
                    E2_m_ba * np.exp(sum_sum) * sum_sum_sign
            ...
            if not return_sum:
                ...
                individual_grads_L = (
                    sum_part_1 + sum_part_2 + sum_part_3 + prior_weight * E1der
                )
