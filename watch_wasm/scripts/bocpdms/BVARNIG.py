...
class BVARNIG(ProbabilityModel):
    ...
    def __init__(
        ...
        if (
            hyperparameter_optimization is not None
            or hyperparameter_optimization is not False
        ):
        ...
        self.generalized_bayes_rld = (
            "kullback_leibler"  # changed from inside detector init
        )
        ...
        if (
            (not self.restriction_sequence is None)
            and (not self.nbh_sequence is None)
            and (not self.padding is None)
        ):
            self.regular_grid = True
        elif (
            (not self.general_nbh_sequence is None)
            and (not self.general_nbh_restriction_sequence is None)
            and (not self.general_nbh_coupling is None)
        ):
            self.regular_grid = False
        elif (
            (self.restriction_sequence is None)
            and (self.nbh_sequence is None)
            and (self.general_nbh_sequence is None)
            and (self.general_nbh_restriction_sequence is None)
        ):
            ...
            raise SystemExit(
                "Your neighbourhood specifications "
        ...
        that beta_0 = 0."""
        if self.prior_mean_beta is None or self.num_regressors != np.size(
        ...
        that V_0 = 100*I."""
        if (
            self.prior_var_beta is None
            or self.num_regressors != prior_var_beta.shape[0]
            or self.num_regressors != prior_var_beta.shape[1]
        ):
        ...
        if self.regular_grid:
            ...
            for lag in range(0, int(self.lag_length)):
                ...
                if restriction == 0:
                    if nbh == 0:
                        ...
                        endo_vars.append(["center", "top", "left", "right", "bottom"])
                    elif nbh == 8:
                        endo_vars.append(
                            [
                                "center",
                    ...
                    if nbh == 0:
                        ...
                        endo_vars.append(
                            ["center", "4_outer_nbh_res", "4_inner_nbh_res"]
                        )
                elif restriction == 8:
                    if nbh == 0:
                        endo_vars.append(["center"])
                        ...
                        print(
                            "Warning: Restriction = 8, which is not fully implemented"
                        )
            ...
            for lag in range(0, int(self.lag_length)):
                ...
                for location in range(0, self.S1 * self.S2):
                    ...
                    new_endo_vars_entry.append(
                        "general_nbh_" + str(lag) + "_" + "center" + "_" + str(location)
                    )
            ...
            for lag in range(0, int(self.lag_length)):
                ...
                for nbh_index in relevant_nbh_indices:
                    new_endo_vars_entry.append(
                        "general_nbh_" + str(lag) + "_" + str(nbh_index)
                    )
                endo_vars.append(new_endo_vars_entry)
    ...
    def get_lag_counts(self):
        ...
        if self.regular_grid:
            ...
            for lag in range(0, self.lag_length):
                self.lag_counts.append(
                    last_count + ((len(self.general_nbh_restriction_sequence[lag]) + 1))
                )
            ...
            for lag in range(0, self.lag_length):
                self.lag_counts.append(
                    ...
                    - self.sum_empty_nbhs_per_lag[lag]
                )
                last_count = last_count + (
                    -self.sum_empty_nbhs_per_lag[lag]
                    + (len(self.general_nbh_restriction_sequence[lag]) + 1)
        ...
        if self.regular_grid:
            ...
            for i in range(0, self.lag_length - 1):
                self.extraction_list = (
                    ...
                    + [False]
                    * int(endo_regressors_per_lag[i] - endo_regressors_per_lag[i + 1])
                )
            """STEP 2A: The last lag of X_t-1 will 'slide out' of sight, so it 
                       definitely is not needed for X_t anymore."""
            self.extraction_list += [False] * endo_regressors_per_lag[
                self.lag_length - 1
            ]
            ...
            for lag in range(0, self.lag_length - 1):
                ...
                num_discarded = (
                    -num_retained + 1 + len(self.general_nbh_restriction_sequence[lag])
                )
                per_location += [True] * num_retained + [False] * num_discarded
            ...
            self.extraction_list[self.num_exo_regressors :] = np.array(
                self.extraction_list
            )[np.where(np.array(self.empty_nbhs) == False)].tolist()
    ...
    def reinstantiate(self, a=None, b=None):
        ...
        clone_model = BVARNIG(
            ...
            nbh_sequence_exo=nbh_sequence_exo,
    ...
    def initialization(
        ...
        """
        self.XX_rt = np.zeros(
            shape=(2, self.num_regressors, self.num_regressors)
        )  # 2 for r=-1 and r=0
        ...
        self.beta_XX_beta_rt[0] = self.beta_XX_beta_rt[1] = np.inner(
            np.matmul(self.beta_rt[0, :], self.XX_rt[0, :]), self.beta_rt[0, :]
        )
        ...
        small_matrix_inv = np.linalg.inv(
            ...
            + np.matmul(
                (self.X_tp1),
                np.matmul(self.M_inv_1_rt[0, :, :], np.transpose(self.X_tp1)),
            )
        )
        ...
        self.M_inv_2_rt[0, :, :] = self.M_inv_2_rt[1, :, :] = self.M_inv_1_rt[
            0, :, :
        ] - np.matmul(
            (M_inv_1_x_X_tp1),
            np.matmul(small_matrix_inv, np.transpose(M_inv_1_x_X_tp1)),
        )
        ...
        C_0_inv = (a_ / b_) * (
            np.identity(self.S1 * self.S2)
            - np.matmul(
                self.X_t, np.matmul(self.M_inv_1_rt[0, :, :], np.transpose(self.X_t))
            )
        )
        if b_ < 0:
            log_det = np.nan
            ...
            log_det = (
                (self.S1 * self.S2) * (np.log(b_) - np.log(a_))
                + self.D_inv_log_det
                - self.log_det_1_rt[0]
            )
        ...
        self.model_log_evidence = np.log(model_prior) + BVARNIG.mvt_log_density(
            resid, C_0_inv, log_det, 2 * a_, self.non_spd_alerts
        )
    ...
    def compute_X_XX_XY_YY(
        self, Y0, X0_endo, X0_exo, padding_columns=None, compute_XY=True
    ):
        ...
        for i in range(0, self.num_regressors):
            ...
            if i <= (self.num_exo_regressors - 1):
                ...
                if padding_columns is None:
                    data_vector1 = self.get_endo_regressors(
                        self.all_vars[i], lag_count1, X0_endo
                    )
                else:
                    data_vector1 = self.get_endo_regressors(
                        self.all_vars[i], lag_count1, X0_endo, padding_columns[i, :]
                    )
    ...
    def evaluate_predictive_log_distribution(self, y, t):
        ...
        for r in range(0, run_length_num):
            ...
            b_ = self.b + 0.5 * (
                self.b0_D_inv_b0 + self.YY_rt[r] - self.beta_XX_beta_rt[r]
            )
            self.C_t_inv[r + 1, :, :] = np.identity(self.S1 * self.S2) - np.matmul(
                self.X_tp1,
                np.matmul(self.M_inv_2_rt[r, :, :], np.transpose(self.X_tp1)),
            )
    ...
    def get_log_integrals_power_divergence(self):
        ...
        C1 = (1.0 + self.alpha_rld) * (gammaln(0.5 * (nu_1 + p)) - gammaln(0.5 * nu_1))
        C2 = gammaln(0.5 * (nu_2 + p)) - gammaln(0.5 * nu_2)
    ...
    def evaluate_log_prior_predictive(self, y, t):
        ...
        return min(
            ...
            BVARNIG.mvt_log_density(
                resid, (self.a / self.b) * self.C_t_inv_r0, log_det, 2 * self.a, True
            ),
        )
    ...
    def save_NLL_fixed_pars(self, y, t):
        ...
        for r in range(0, run_length_num):
            ...
            """normal evaluation"""
            log_densities[r] = stats.multivariate_normal.logpdf(resid, cov=cov_mat)
    ...
    def update_predictive_distributions(
        ...
        self.XX_old, self.XY_old, self.X_t_old = (
            self.XX.copy(),
            self.XY.copy(),
            self.X_t.copy(),
        )
    ...
    def regressor_cross_product_updates(
        self, y_t, y_tm1, x_exo, t, padding_column=None, rt_updates=True
    ):
        ...
        if self.has_lags and self.lag_length > 1:
            self.X_t[:, self.insertion_position :] = self.X_t[:, self.extraction_list]
            self.XX[self.insertion_position :, self.insertion_position :] = self.XX[
                self.extraction_list, :
            ][:, self.extraction_list]
        ...
        if (
            not (self.restriction_sequence is None)
            or self.general_nbh_coupling == "strong coupling"
        ):
            num_new_vars = len(self.endo_vars[0]) + self.num_exo_regressors
            new_vars = sum(self.exo_vars, []) + self.endo_vars[0]
        elif self.general_nbh_coupling == "weak coupling":
            new_vars = sum(self.exo_vars, []) + sum(
                self.endo_vars[: self.S1 * self.S2], []
            )
        ...
        for regressor_code in new_vars:  # sum(self.exo_vars,[]) + self.endo_vars[0]:
            ...
            for j in range(0, num_new_vars):
                if i <= j:
                    if j <= self.num_exo_regressors - 1:
                        x_j = self.get_exo_regressors(self.all_vars[j], j, x_exo)
                    elif self.has_lags:
                        x_j = self.get_endo_regressors(
                            self.all_vars[j],
                            1,
                            y_tm1.reshape(1, self.S1, self.S2),
                            padding_column,
    ...
    def get_x_new(self, y_t, x_exo_tp1, t, padding_column=None):
        ...
        if self.has_lags and self.lag_length > 1:
            ...
            x_new[:, self.insertion_position :] = self.X_t[
                :, self.extraction_list
            ].copy()
        else:
            x_new = np.zeros((self.S1 * self.S2, self.num_regressors))
        ...
        for regressor_code in all_codes:
            ...
            if i <= self.num_exo_regressors - 1:
                ...
                x_i = self.get_endo_regressors(
                    regressor_code, 1, y_t.reshape(1, self.S1, self.S2), padding_column
                )
    ...
    def updates(self, t):
        ...
        self.M_inv_2_rt = np.zeros(
            (run_length_num, self.num_regressors, self.num_regressors)
        )
        self.log_det_2_rt = np.zeros(run_length_num)

        for r in range(0, run_length_num):
            """Compute small_matrix for r, and get inverse + det"""
            M_inv_x_X_tp1 = np.matmul(
                self.M_inv_1_rt[r, :, :], np.transpose(self.X_tp1)
            )
            small_matrix_inv = np.linalg.inv(
                np.identity(self.S1 * self.S2)
                + np.matmul(np.transpose(M_inv_x_X_tp1), np.transpose(self.X_tp1))
            )

            """Update M_2_inv"""
            self.M_inv_2_rt[r, :, :] = self.M_inv_1_rt[r, :, :] - np.matmul(
                (M_inv_x_X_tp1),
                np.matmul(small_matrix_inv, np.transpose(M_inv_x_X_tp1)),
            )
    ...
    def post_QR_updates(self, t):
        ...
        run_length_num = self.retained_run_lengths.shape[0]
        for r in range(0, run_length_num):
            self.M_inv_2_rt[r, :, :] = solve_triangular(
                a=self.R_rt[r, :, :],
                b=np.transpose(self.Q_rt[r, :, :]),
                check_finite=False,
            )
        self.log_det_2_rt = np.sum(
            np.log(np.abs(np.diagonal(self.R_rt, axis1=1, axis2=2))), axis=1
        )
    ...
    def compute_betas(self, t):
        ...
        run_length_num = self.retained_run_lengths.shape[0]
        self.beta_rt = np.insert(self.beta_rt, 0, np.zeros(self.num_regressors), axis=0)
        ...
        for r in range(0, run_length_num):
            ...
            self.beta_XX_beta_rt[r] = np.inner(
                self.beta_rt[r, :], np.matmul(self.XX_rt[r, :, :], self.beta_rt[r, :])
            )
    ...
    def QR_loop(self, Q0, R0, X):
        ...
        while end_point != self.S1 * self.S2:
            ...
            end_point = min(
                (current_count + 1) * self.num_regressors, self.S1 * self.S2
            )
            current_range = range(start_point, end_point)
            Q0, R0 = linalg.qr_update(
                Q0,
                R0,
                np.transpose(X[current_range, :]),
                np.transpose(X[current_range, :]),
                check_finite=False,
            )  # TODO: implement qr_update
    ...
    def get_posterior_variance(self, t, r_list=None):
        ...
        for r in range(0, run_length_num):
            ...
            post_var[r, :, :] = (b_ / a_) * (
                ...
                + np.matmul(
                    self.X_tp1,
                    np.matmul(self.M_inv_1_rt[r, :, :], np.transpose(self.X_tp1)),
                )
            )
        return post_var
    ...
    def mvt_log_density(y_flat, prec, log_det, df, prior=False, alerts=False):
        ...
        if log_term < 0 or np.isnan(log_det):
            ...
            if not prior and p > 1:
                ...
                # print(prec)
                try:
                    prec = NPD.nearestPD(prec) + np.identity(prec.shape[0]) * max(
                        df * nu, max(25, p)
                    )
                except (
                    ValueError,
                    np.linalg.LinAlgError,
                ) as e:  # np.linalg.LinAlgError
    ...
    def get_endo_regressors(self, regressor_code, lag, data, padding_column=None):
        ...
        *padding_column* is only for the demo object BVAR_NIG_CSurf: It is the
        ...
        if padding == 0 or padding == "zero":
            ...
            padding_corners = weight * np.sum(padding_row) + (1 - weight) * np.sum(
                padding_col
            )
            ...
            if padding.split("_")[-1] == "rhs":
                ...
                padding_corner_lhs = padding_corners = weight * np.sum(padding_row) + (
                    1 - weight
                ) * np.sum(padding_lhs)
            else:
                """get padding for cols as usual + specific one for rhs, lhs"""
                ...
                padding_corner_rhs = padding_corners = weight * np.sum(padding_row) + (
                    1 - weight
                ) * np.sum(padding_rhs)
        ...
        if regressor_code == "intercept":
            ...
            if padding.split("_")[-1] == "lhs":
                """Insert the padding column passed to this function"""
                data_vector = np.insert(
                    data[lag, :, 1:], self.S2 - 1, padding_lhs, axis=1
                )
            else:
                """Take the row averages as padding"""
                data_vector = np.insert(
                    data[lag, :, 1:], self.S2 - 1, padding_col, axis=1
                )
            ...
            if padding.split("_")[-1] == "lhs":
                ...
                data_vector = (
                    np.insert(data[lag, :, :-1], 0, padding_col, axis=1)
                ...
                data_vector = (
                    np.insert(data[lag, :, :-1], 0, padding_rhs, axis=1)
        ...
        elif (regressor_code == "4_outer_nbh_res") or (regressor_code == "8_nbh_res"):
            if padding.split("_")[-1] == "lhs":
                # pad with real data on the right
        ...
        if regressor_code.split("_")[0] == "general":
            ...
            if self.general_nbh_coupling == "strong coupling":
                ...
                if num_group == "center":
                    ...
                    {N_i(s): s \in S1*S2}"""
                    relevant_nbhs = [
                        item[num_group] for item in self.general_nbh_sequence
                    ]
                    ...
                    data_vector = np.array(
                        [sum(dat[relevant_nbh]) for relevant_nbh in relevant_nbhs]
                    )
                ...
                if regressor_code.split("_")[-2] == "center":
                    ...
                    # print(lag)
                    # print(nbh_index)
                    relevant_nbh_index = self.general_nbh_restriction_sequence[lag2][
                        nbh_index
                    ]
                    ...
                    relevant_nbh = self.general_nbh_sequence[location][
                        relevant_nbh_index
                    ]
                    ...
                    if relevant_nbh:
                        data_vector[location] = sum(
                            data[lag, :, :].flatten()[relevant_nbh]
                        )
    ...
    def prior_update(self, t, model_specific_rld):
        ...
        if True:
            self.prior_mean_beta = np.sum(
                self.beta_rt * np.exp(model_specific_rld[filled_in, np.newaxis]), axis=0
            )
            self.D_inv_b0 = np.matmul(self.D_inv, self.prior_mean_beta)
            self.b0_D_inv_b0 = np.inner(self.prior_mean_beta, self.D_inv_b0)
        ...
        if True:
            ...
            b_vec = self.b + 0.5 * (
                self.b0_D_inv_b0 + self.YY_rt + self.beta_XX_beta_rt
            )
    ...
    def differentiate_predictive_log_distribution(self, y, t, run_length_log_distro):
        ...
        resids = np.insert(
            np.matmul(self.X_t, self.beta_rt[:, :, np.newaxis])
        ...
        products = np.sum(resids * np.matmul(self.C_t_inv, resids), axis=1).reshape(
            run_length_num + 1
        )
        ...
        self.evaluated_log_predictive_distr. We need to take speciaal care 
        ...
        log_gradients_a_sign = np.sign(inside6) * np.sign(
            log_posterior_predictive_gradients_a_val
        )
        ...
        log_gradients_sign = np.array(
            [log_gradients_a_sign, log_gradients_b_sign]
        ).reshape(num_params, run_length_num + 1)
        log_gradients_val = np.array(
            [log_gradients_a_val, log_gradients_b_val]
        ).reshape(num_params, run_length_num + 1)
    ...
    def caron_hyperparameter_optimization(self, t, gradient, step_size):
        ...
        if True:  # Might want to add different options w.r.t step size
            ...
            explosive behaviour."""
            dampener = 1.0 / t
            increment_a = sign_a * max(
                self.a * dampener * min_step_scale,
                min(
                    self.a * dampener * max_step_scale,
                    np.abs(step_size_abs * gradient[0]),
                ),
            )
            increment_b = sign_b * max(
                self.a * dampener * min_step_scale,
                min(
                    self.b * dampener * max_step_scale,
        ...
        self.a = min(max(self.a + increment_a, 1.0), pow(10, pow(10, 2)))
        self.b = min(max(self.b + increment_b, pow(10, -10)), pow(10, pow(10, 2)))
    ...
    def get_one_step_ahead_log_loss_derivatives_power_divergence(self):
        ...
        nu_2_vec = np.maximum(
            nu_1_vec * p + nu_1_vec * self.alpha_rld + nu_1_vec, 1.005
        )
        ...
        f_1_log = (1.0 + self.alpha_rld) * (
            gammaln(0.5 * (nu_1_vec + p)) - gammaln(0.5 * nu_1_vec)
        )
        f_2_log = gammaln(0.5 * (nu_2_vec)) - gammaln(0.5 * nu_2_vec + p)
        ...
        expr_1 = gammaln(0.5 * (nu_1_vec + p)) - gammaln(0.5 * nu_1_vec)
        ...
        digamma_expr_2_A = scipy.special.psi(0.5 * (nu_2_vec))
        expr_2_A_sign = np.sign(digamma_expr_2_A)
        expr_2_A = (
            gammaln(0.5 * (nu_2_vec))
            + np.log(expr_2_A_sign * digamma_expr_2_A)
            + np.log(0.5 * (nu_1_vec + p))
            - gammaln(0.5 * (nu_2_vec + p))
        )

        digamma_expr_2_B = scipy.special.psi(0.5 * (nu_2_vec + p))
        expr_2_B_sign = np.sign(digamma_expr_2_B)
        expr_2_B = (
            -gammaln(0.5 * (p + nu_2_vec))
        ...
        f_2_der_log, f_2_der_sign = scipy.special.logsumexp(
            a=np.array([expr_2_A, expr_2_B]),
            b=np.array([expr_2_A_sign, expr_2_B_sign]),
            return_sign=True,
            axis=0,
        )
        ...
        expr_3 = -(
            0.5 * p * (np.log(nu_1_vec) + np.log(np.pi))
            + self.predictive_variance_log_det
        )
        ...
        log_integral_derivatives_val, log_integral_derivatives_sign = scipy.special.logsumexp(
            a=np.array([f_1_full_expr, f_2_full_expr, f_3_full_expr]),
            b=np.array([f_1_der_sign, f_2_der_sign, f_3_der_sign]),
            return_sign=True,
            axis=0,
        )
       ...
       where we have computed Int_der(alpha) in log form in STEP 2."""
        ...
        predictive_log_probs = np.insert(
            self.one_step_ahead_predictive_log_probs.copy(), 0, self.r0_log_prob
        )

        log_constant = (1.0 / self.alpha_rld) * np.power(
            np.exp(predictive_log_probs), self.alpha_rld
        ...
        expr_val, expr_sign = scipy.special.logsumexp(
            a=np.array([expr_1_val, expr_2_val]),
            b=np.array([expr_1_sign, expr_2_sign]),
            return_sign=True,
            axis=0,
        )
    ...
    def turner_hyperparameter_optimization(self, step_size):
        ...
        if True:
            ...
            D1 = (
                min(
                    max(np.abs(np.dot(dif_old_val, dif_old_grad)), pow(10, 5)),
                    pow(10, -5),
                )
                * D1_sign
            )
