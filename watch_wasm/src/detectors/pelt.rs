...
impl CostRbf {
    ...
    fn error(&self, start: usize, end: usize) -> f64 {
        ...
        let ps = &self.psum;
        let total =
            ps[end][end] - ps[start][end] - ps[end][start] + ps[start][start];
...
impl PELT {
    ...
    fn segment(&self) -> HashMap<(usize, usize), f64> {
        ...
        let mut idxs: Vec<usize> =
            (0..self.n).step_by(self.jump).filter(|&k| k >= self.min_size).collect();
        idxs.push(self.n);
        ...
        for &bkp in &idxs {
            ...
            if let Some(best) = subs
                .into_iter()
                .min_by(|a, b| a.values().sum::<f64>().total_cmp(&b.values().sum::<f64>()))
            {
    ...
    fn detect_internal(&mut self, rows: &[Vec<f64>]) -> Vec<usize> {
        self.fit(rows);
